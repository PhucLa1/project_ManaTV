...
{
    public partial class FrmListOrigins : Form
    {
        private OriginPresenter _objPresenter;
        private List<BunifuButton> _buttonList;
        public Form FRM_LAYOUT { get; set; }
        public FrmListOrigins()
        {
            _objPresenter = new OriginPresenter();
            InitializeComponent();
            InitForm();
        ...
        private void ListOrigins_Load(object sender, EventArgs e)
        {
        ...
        private void gridData_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (clickedColumn.Name == "actionDetails")
                {
                    ...
                    var formObj = new FormOrigin("details");
                ...
                if (clickedColumn.Name == "actionUpdate")
                {
                    var id = int.Parse(selectedRow.Cells["ID"].Value.ToString());
                    var selectedObj = _objPresenter.GetById(id);
                    var formObj = new FormOrigin("update", _objPresenter);
                    formObj.SetData(selectedObj);
        ...
        {
            var formobj = new FormOrigin("add", _objPresenter);
        ...
        //FORM EVENT
        private void RenderListData(List<Origin> lstobj)
        {
            gridData.Rows.Clear();
