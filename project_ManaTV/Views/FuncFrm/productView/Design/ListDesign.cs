...
{
    public partial class FrmListDesigns : Form
    {
        private DesignPresenter _objPresenter;
        private List<BunifuButton> _buttonList;
        public Form FRM_LAYOUT { get; set; }
        public FrmListDesigns()
        {
            _objPresenter = new DesignPresenter();
            InitializeComponent();
            InitForm();
        ...
        private void ListDesigns_Load(object sender, EventArgs e)
        {
        ...
        private void gridData_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (clickedColumn.Name == "actionDetails")
                {
                    ...
                    var formObj = new FormDesign("details");
                ...
                if (clickedColumn.Name == "actionUpdate")
                {
                    var id = int.Parse(selectedRow.Cells["ID"].Value.ToString());
                    var selectedObj = _objPresenter.GetById(id);
                    var formObj = new FormDesign("update", _objPresenter);
                    formObj.SetData(selectedObj);
        ...
        {
            var formobj = new FormDesign("add", _objPresenter);
        ...
        //FORM EVENT
        private void RenderListData(List<Design> lstobj)
        {
            gridData.Rows.Clear();
