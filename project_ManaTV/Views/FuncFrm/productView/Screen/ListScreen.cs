...
{
    public partial class FrmListScreens : Form
    {
        private ScreenPresenter _objPresenter;
        private List<BunifuButton> _buttonList;
        public Form FRM_LAYOUT { get; set; }
        public FrmListScreens()
        {
            _objPresenter = new ScreenPresenter();
            InitializeComponent();
            InitForm();
        ...
        private void ListScreens_Load(object sender, EventArgs e)
        {
        ...
        private void gridData_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (clickedColumn.Name == "actionDetails")
                {
                    ...
                    var formObj = new FormScreen("details");
                ...
                if (clickedColumn.Name == "actionUpdate")
                {
                    var id = int.Parse(selectedRow.Cells["ID"].Value.ToString());
                    var selectedObj = _objPresenter.GetById(id);
                    var formObj = new FormScreen("update", _objPresenter);
                    formObj.SetData(selectedObj);
        ...
        {
            var formobj = new FormScreen("add", _objPresenter);
        ...
        //FORM EVENT
        private void RenderListData(List<Screen> lstobj)
        {
            gridData.Rows.Clear();
